...
static I32 DD_dumpfile _((FILE *pio_file, SV *val, char *name, STRLEN namelen,
		      HV *seenhv, AV *postav, I32 *levelp, I32 indent,
		      SV *pad, SV *xpad, SV *apad, SV *sep, int top));
...
static I32
DD_dumpfile(pio_file,val, name, namelen, seenhv, postav,
	levelp, indent, pad, xpad, apad, sep, top)
FILE    *pio_file;
SV	*val;
char	*name;
STRLEN	namelen;
...
int     top;
{
#define CCH_TMPBUF 4096
    char tmpbuf[CCH_TMPBUF]; // FIXGJB: this hard limit can cause problems
    U32 i;
    char *c, *r, *realpack, id[128];
    SV **svp;
    SV *sv;
...
// This stuff gets inserted later, but we must print it now
    if (top) {
	PerlIO_puts(pio_file,name);
	PerlIO_puts(pio_file," = ");
...
//			sv_insert(valstr, 0, 0, SvPVX(name), SvCUR(name));
    ...
    if (SvGMAGICAL(val))
        mg_get(val);
    if (val == &sv_undef || !SvOK(val)) {
	PerlIO_puts(pio_file,"undef");
//	sv_catpvn(retval, "undef", 5);
	PerlIO_flush(pio_file);
	return 1;
    }
    if (SvROK(val)) {
...
//			sv_catpvn(retval, "''", 2);
			...
			if ((SvPVX(othername))[0] == '\\' &&
			    (SvPVX(othername))[1] == name[0]) {
			    PerlIO_puts(pio_file, SvPVX(othername)+1);
//			    sv_catpvn(retval, SvPVX(othername)+1, SvCUR(othername)-1);
			}
			else {
			    PerlIO_putc(pio_file, *name);
			    PerlIO_putc(pio_file, '{');
			    PerlIO_puts(pio_file, SvPVX(othername));
			    PerlIO_putc(pio_file, '}');
//			    sv_catpvn(retval, name, 1);
//			    sv_catpvn(retval, "{", 1);
//			    sv_catsv(retval, othername);
...
//	    sv_catpvn(retval, "bless( ", 7);
	...
	if (realtype <= SVt_PVBM || realtype == SVt_PVGV) {  /* scalars */
	    if (realpack && realtype != SVt_PVGV) {          /* blessed */ 
		PerlIO_puts(pio_file, "\\($_ = ");
//		sv_catpvn(retval, "\\($_ = ", 7);
		DD_dumpfile(pio_file,ival, "", 0, seenhv, postav,
			levelp,	indent, pad, xpad, apad, sep,0);
		PerlIO_putc(pio_file, ')');
//		sv_catpvn(retval, ")", 1);
	    }
	    else {
...
//		sv_catpvn(retval, "[", 1);
		...
		iname[ilen++] = ']'; iname[ilen] = '\0';
		if (indent >= 3) {
		    PerlIO_puts(pio_file, SvPVX(totpad));
		    PerlIO_puts(pio_file, SvPVX(ipad));
		    PerlIO_putc(pio_file, '#');
		    PerlIO_puts(pio_file, SvPVX(ixsv));
//		    sv_catsv(retval, totpad);
//		    sv_catsv(retval, ipad);
//		    sv_catpvn(retval, "#", 1);
//		    sv_catsv(retval, ixsv);
...
//		sv_catsv(retval, totpad);
		PerlIO_puts(pio_file, SvPVX(ipad));
//		sv_catsv(retval, ipad);
		DD_dumpfile(pio_file,elem, iname, ilen, seenhv, postav,
			    levelp,	indent, pad, xpad, apad, sep,0);
		if (ix < ixmax) {
		    PerlIO_putc(pio_file, ',');
//		    sv_catpvn(retval, ",", 1);
		...
		PerlIO_puts(pio_file,SvPVX(totpad));
//		sv_catsv(retval, totpad);
		PerlIO_puts(pio_file,SvPVX(opad));
//		sv_catsv(retval, opad);
		SvREFCNT_dec(opad);
	    }
	    if (name[0] == '@')
...
//		sv_catsv(retval, totpad);
		PerlIO_puts(pio_file,SvPVX(ipad));
//		sv_catsv(retval, ipad);
		PerlIO_write(pio_file,nkey,klen);
//		sv_catpvn(retval, nkey, klen);
		PerlIO_puts(pio_file," => ");
//		sv_catpvn(retval, " => ", 4);
		if (indent >= 2) {
...
//		sv_catpvn(retval, "}", 1);
	    ...
	    PerlIO_puts(pio_file,"sub { \"DUMMY\" }");
//	    sv_catpvn(retval, "sub { \"DUMMY\" }", 15);
	    if (purity)
		warn("Encountered CODE ref, using dummy placeholder");
	}
...
//	    sv_catpvn(retval, "' )", 3);
		...
		PerlIO_puts(pio_file, "->");
//		sv_catpvn(retval, "->", 2);
		PerlIO_puts(pio_file, SvPVX(toaster));
//		sv_catsv(retval, toaster);
		PerlIO_puts(pio_file, "()");
//		sv_catpvn(retval, "()", 2);
	    }
...
//		r[i] = '\0';
		PerlIO_puts(pio_file,"*{\'");
		Zero(tmpbuf,CCH_TMPBUF,char);
		i += esc_q(tmpbuf,c,i) + 3;
		PerlIO_puts(pio_file,tmpbuf);
		PerlIO_puts(pio_file,"\'}");
	    }
	    else {
...
void
Data_Dumper_DumpxsFd(href, fd, ...)
	SV	*href;
	int fd;
	PROTOTYPE: $$;$$
	PPCODE:
	{
            FILE *pio_file = fdopen(fd,"a");
	    HV *hv;
//	    SV *valstr;
	    HV *seenhv = Nullhv;
		...
		if (items == 4) {
		    ...
		    croak("Usage: Data::Dumper::DumpxsFile(PACKAGE, VAL_ARY_REF, NAME_ARY_REF)");
...
//		valstr = newSVpv("",0);
		for (i = 0; i <= imax; ++i) {
		    ...
		    PerlIO_puts(pio_file,SvPVX(pad));
		    // sv_catsv(retval, pad);
		    // DD_dumpfile takes extra first arg, FILE *,
		    // and no valstr arg, and outputs the "name = " 
		    // stuff that I have commented out just below
		    DD_dumpfile(pio_file,val, SvPVX(name), SvCUR(name), 
			        seenhv, postav,
			        &level, indent, pad, xpad, newapad, sep, 1);
...
//		    }
		    PerlIO_puts(pio_file,";");
		    PerlIO_puts(pio_file,SvPVX(sep));
		    // sv_catsv(retval, sep);
		    if (postlen >= 0) {
			I32 i;
	 	        PerlIO_puts(pio_file,SvPVX(pad));
			...
			for (i = 0; i <= postlen; ++i) {
			    ...
			    if (svp && (elem = *svp)) {
			        PerlIO_puts(pio_file,SvPVX(elem));
