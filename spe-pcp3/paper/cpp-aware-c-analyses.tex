...
interacting with the parser.  \texttt{Parse\-State\-Stack} returns the
...
\begin{figure}[p]
  \begin{center}
    \leavevmode
    \includegraphics[width=0.60\linewidth]{tree-expn.eps}
    \caption{An example macro expansion and the related
      ...
      tree). Uncircled numbers are \texttt{EXPAND\_MACRO} actions, circled
...
\begin{figure}[p]
  \begin{center}
    \leavevmode
    \includegraphics[width=0.65\linewidth]{text-expn.eps}
    \caption{Another view of an example macro expansion and the hooks
    that are called.  Dotted lines are the nestings of macro
    expansions as the algorithm recurses.  Dark solid lines are
    \texttt{EXPAND\_MACRO} actions on the underlined macro invocation,
    light solid lines are \texttt{MACRO\_CLEANUP}s.}
    \label{fig:text-expn}
  \end{center}
\end{figure}
...
for a given expansion corresponds to the sequence of dotted lines
directly above and completely overlapping the dark solid line representing
that expansion.  Similarly, the list \texttt{MacroExpansionHistory()}
returns can be visualized as the stack of dark solid expansion
lines directly above the expansion in question (those that have not
already been paired with a light solid line representing the completion of
their expansion).  
...
\begin{figure}[p]
  \begin{center}
    \leavevmode
    \includegraphics[width=0.55\linewidth]{emacs-xform-view.eps}
...
Fellowship and by NSF Grant CCR-9506779. Thanks to Michael Ernst, Craig
Kaplan, Brian Michalowski, Gail Murphy, and Douglas Zongker for their
thoughtful comments on earlier revisions of this paper.
...
\item[\hook{IFDEF\_MACRO}{..}, \hook{IFDEF\_LOOKUP\_MACRO}{\$mname,\$fDefined}] ~ \\
  ...
  \ppd{if FOO} is encountered.  \shook{IFDEF\_\-LOOKUP\_\-MACRO} is invoked
